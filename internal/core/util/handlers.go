...
func EnglishPriceHandler(p string) (price string) {
	...
	price = strings.ReplaceAll(price, "指定席", "Reserved Seating")
	price = strings.ReplaceAll(price, "このライブハウスのイベントの値段にアクセスできません。ライブのリンクをチェックしてください。", "Cannot access prices for lives at this venue. Please check live link.")
	...
	price = strings.ReplaceAll(price, "D代別", "Drink must be purchased separately")
	...
	price = strings.ReplaceAll(price, "別途必要", "Must be purchased separately")
	...
	price = strings.ReplaceAll(price, "入場時", "When entering")
	...
	price = strings.ReplaceAll(price, "後方エリア", "Rear area")
	price = strings.ReplaceAll(price, "優先", "Priority entry")
	price = strings.ReplaceAll(price, "代金", "Price")
